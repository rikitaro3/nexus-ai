...
jest.mock('../../src/main/utils/quality-gates', () => {
  ...
  function buildResult(mode: 'auto' | 'manual' | 'bulk', contextPath?: string | null) {
    ...
    return {
      ...
      autofix: null,
      repoDiff: null,
      logPath: path.join('tools', 'nexus', 'logs', 'quality-gates', `integration-${mode}.json`),
      relativeLogPath: path.join('tools/nexus/logs/quality-gates', `integration-${mode}.json`),
      rawOutput: '',
      stderr: ''
    };
  }
...
describe('RulesWatcherController integration', () => {
  ...
  it('emits a diff summary and validation snapshot when rules change', async () => {
    ...
    try {
      const rulePath = path.join(projectRoot, 'tools', 'nexus', 'docs', 'GATES', 'document.mdc');
