...
// テスト時のuserData切り替え（できるだけ早い行で）
const userDataFromEnv = process.env.ELECTRON_USER_DATA_DIR;
if (userDataFromEnv) {
  app.setPath('userData', userDataFromEnv);
  logger.info('Using custom userData for testing', { path: userDataFromEnv });
}
...
const PROJECT_ROOT = process.env.NEXUS_PROJECT_ROOT 
  ...
  : (() => {
      ...
      for (let i = 0; i < 10; i++) {
        const marker = path.join(searchDir, 'context.mdc');
...
const NEXUS_DIR = PROJECT_ROOT;
const PROMPTS_JSON_PATH = path.join(PROJECT_ROOT, 'prompts.json');
...
function createWindow() {
  ...
  const rendererPath = path.join(PROJECT_ROOT, 'src', 'renderer');
  const preloadPath = path.join(appPath, '..', 'preload', 'preload.js');
  
  // Debug: preload path verification
  logger.info('Creating window', { 
    appPath, 
    preloadPath, 
    preloadExists: fs.existsSync(preloadPath),
    distPreloadPath: path.join(process.cwd(), 'dist', 'src', 'preload', 'preload.js'),
    distPreloadExists: fs.existsSync(path.join(process.cwd(), 'dist', 'src', 'preload', 'preload.js'))
  });
  ...
  mainWindow = new BrowserWindow({
    ...
    webPreferences: {
      ...
      contextIsolation: true,  // セキュリティのためにtrueを維持
      sandbox: false,  // Preloadスクリプトが正しく動作するようにsandboxをfalseに設定
      preload: preloadPath
    }
  });
...
ipcMain.handle('dialog:selectContextFile', async () => {
  ...
  const result = await dialog.showOpenDialog(mainWindow, {
    ...
    filters: [
      { name: 'Context Files', extensions: ['mdc', 'md', 'yaml', 'yml'] }
    ],
    properties: ['openFile']
  });
...
ipcMain.handle('tasks:readJson', async () => {
  try {
    ...
    const target = path.join(repoRoot, 'tasks.json');
...
ipcMain.handle('tasks:readRecommendationHistory', async () => {
  try {
    const repoRoot = getRepoRoot();
    const target = path.join(repoRoot, 'task-recommendations-history.json');
    if (!fs.existsSync(target)) {
...
// YAML parsing
ipcMain.handle('yaml:parse', async (event, content: string) => {
  try {
