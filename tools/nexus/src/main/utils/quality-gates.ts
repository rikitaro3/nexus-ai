...
const LOG_DIR = path.join('logs', 'quality-gates');
...
async function resolveScriptPath(projectRoot: string): Promise<string> {
  const scriptPath = path.join(projectRoot, 'scripts', 'validate-docs-gates.js');
  await fs.access(scriptPath);
  return scriptPath;
}

async function resolveAutofixScriptPath(projectRoot: string): Promise<string> {
  const scriptPath = path.join(projectRoot, 'scripts', 'apply-docs-gates.js');
  await fs.access(scriptPath);
  return scriptPath;
}
