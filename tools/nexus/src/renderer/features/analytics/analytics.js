(function initAnalyticsDashboard() {
  ...
  function renderGateHistory(qualityGates) {
    ...
    thead.innerHTML = '<tr><th>日時</th><th>Pass Rate</th><th>DOC-01</th><th>DOC-02</th><th>DOC-03</th><th>DOC-04</th></tr>';
    ...
    for (const entry of history) {
      ...
      row.innerHTML = `
        ...
        <td>${(Number(entry.passRate) * 100).toFixed(1).replace(/\.0$/, '')}%</td>
        <td>${totals['DOC-01'] ?? 0}</td>
        <td>${totals['DOC-02'] ?? 0}</td>
        <td>${totals['DOC-03'] ?? 0}</td>
        <td>${totals['DOC-04'] ?? 0}</td>
      `;
