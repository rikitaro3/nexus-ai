...
(async function initDocsNavigator() {
  // Load YAML parser from preload
  const yaml = window.yaml;
  if (!yaml) {
    console.warn('[Docs Navigator] YAML parser not available');
  }
  ...
  try {
    ...
    if (customContextPath) {
      ...
      contextPath = contextToUse === 'nexus' ? 'context.mdc' : '.cursor/context.mdc';
      select.addEventListener('change', () => {
        const v = select.value;
        localStorage.setItem('nexus.context', v);
        const nextPath = v === 'nexus' ? 'context.mdc' : '.cursor/context.mdc';
    ...
    if (!ctxRes.success && contextPath !== 'context.mdc' && !customContextPath) {
      console.warn('[Docs Navigator] Context read failed, trying Nexus fallback...', ctxRes.error);
      const fallbackPath = 'context.mdc';
    ...
    if (!ctxRes.success) {
      ...
      console.error('[Docs Navigator] Failed to load context file. Tree view will not work until a valid context file is selected in Settings.');
    } else {
      contextLoaded = true;
      ctx = ctxRes.content;
      ...
      if (yaml && typeof yaml.load === 'function') {
        try {
          const yamlResult = await yaml.load(ctx);
          if (yamlResult && yamlResult.contextMap) {
            parsedYaml = yamlResult;
            console.log('[Docs Navigator] YAML format detected');
            for (const cat of parsedYaml.contextMap) {
            ...
            console.log('[Docs Navigator] Parsed YAML entries:', entries.length, 'categories:', categoryOrder);
          }
        } catch (yamlError) {
          console.log('[Docs Navigator] Not YAML format, trying Markdown', yamlError);
        }
      }
      
      // Fallback to Markdown format if YAML parsing failed
      if (!parsedYaml?.contextMap) {
        ...
        if (mapSection) {
          ...
          for (const line of lines) {
            ...
            // Support both "…" and other separator patterns
            let itemMatch = line.match(/^\-\s+([^\s].*?)\s+…\s+(.*)$/);
            if (!itemMatch) {
              // Try alternative format: "- path … description" or "- path … description with multiple words"
              itemMatch = line.match(/^\-\s+(.+?)\s+…\s+(.*)$/);
            }
            if (itemMatch && currentCat) {
              const path = itemMatch[1].trim();
              const desc = itemMatch[2].trim();
              // Only add entries with valid paths (exclude category headers and empty lines)
              if (path && !path.startsWith('#') && path.length > 0) {
                entries.push({ category: currentCat, path, desc });
              }
            }
          }
          console.log('[Docs Navigator] Parsed Markdown entries:', entries.length, 'categories:', categoryOrder);
          if (entries.length === 0) {
            console.warn('[Docs Navigator] Map section found but no entries were parsed. Check context file format.');
            console.warn('[Docs Navigator] Sample line from mapSection:', mapSection.split('\n').slice(0, 5));
          }
        } else {
          console.warn('[Docs Navigator] Map section not found in context');
          console.warn('[Docs Navigator] Context file may not have "## Context Map" section or format is incorrect.');
        }
      }
  ...
  async function renderTree() {
    ...
    if (entriesData.length === 0) {
      console.error('[renderTree] No entries available. Context file may not be loaded.');
      tView.innerHTML = `<div class="tree-error">コンテキストファイルが読み込まれていません。<br/>Context Fileが正しく設定されているか確認してください。</div>`;
      setTreeStatus('✗ エラー: コンテキストファイル未読み込み', 'error');
      return;
    }
    ...
    try {
      ...
      if (nodes.size === 0) {
        const msg = 'Breadcrumbs情報が見つかりませんでした。ドキュメントにBreadcrumbsセクションが含まれているか確認してください。';
        console.warn('[renderTree]', msg);
        tView.innerHTML = `<div class="tree-error">${msg}</div>`;
        setTreeStatus('✗ Breadcrumbs未検出', 'error');
      } else if (rootNodes.length === 0) {
        const msg = `ノードは${nodes.size}個検出されましたが、ルートノードが見つかりませんでした。<br/>全ノードがupstream/downstreamを持っている可能性があります。`;
        console.warn('[renderTree]', msg);
        tView.innerHTML = `<div class="tree-error">${msg}</div>`;
        setTreeStatus('✗ ルートノード未検出', 'error');
      } else {
